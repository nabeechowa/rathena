...
int64 battle_calc_damage(struct block_list *src,struct block_list *bl,struct Damage *d,int64 damage,uint16 skill_id,uint16 skill_lv)
{
	...
	if (bl->type == BL_MOB) { // Reduces damage received for Green Aura MVP
		...
		if (md && md->damagetaken1 != 100)
			damage = i64max(damage * md->damagetaken1 / 100, 1);
	}
	if (bl->type == BL_MOB) { // Reduces damage received for Green Aura MVP
		mob_data *md = BL_CAST(BL_MOB, bl);
	...
	if (bl->type == BL_MOB) { // Reduces damage received for Green Aura MVP
		...
		if (md && md->damagetaken3 != 100)
			damage = i64max(damage * md->damagetaken3 / 100, 1);
	}
	if (bl->type == BL_MOB) { // Reduces damage received for Green Aura MVP
		mob_data *md = BL_CAST(BL_MOB, bl);

		if (md && md->damagetaken4 != 100)
			damage = i64max(damage * md->damagetaken4 / 100, 1);
	}
	if (bl->type == BL_MOB) { // Reduces damage received for Green Aura MVP
		mob_data *md = BL_CAST(BL_MOB, bl);

		if (md && md->damagetaken5 != 100)
			damage = i64max(damage * md->damagetaken5 / 100, 1);
	}
	if (bl->type == BL_MOB) { // Reduces damage received for Green Aura MVP
		mob_data *md = BL_CAST(BL_MOB, bl);

		if (md && md->damagetaken6 != 100)
			damage = i64max(damage * md->damagetaken6 / 100, 1);
	}
	if (bl->type == BL_MOB) { // Reduces damage received for Green Aura MVP
		mob_data *md = BL_CAST(BL_MOB, bl);

		if (md && md->damagetaken7 != 100)
			damage = i64max(damage * md->damagetaken7 / 100, 1);
	}
	if (bl->type == BL_MOB) { // Reduces damage received for Green Aura MVP
		mob_data *md = BL_CAST(BL_MOB, bl);

		if (md && md->damagetaken8 != 100)
			damage = i64max(damage * md->damagetaken8 / 100, 1);
	}
	if (bl->type == BL_MOB) { // Reduces damage received for Green Aura MVP
		mob_data *md = BL_CAST(BL_MOB, bl);

		if (md && md->damagetaken9 != 100)
			damage = i64max(damage * md->damagetaken9 / 100, 1);
	}
	if (bl->type == BL_MOB) { // Reduces damage received for Green Aura MVP
		mob_data *md = BL_CAST(BL_MOB, bl);

		if (md && md->damagetaken10 != 100)
			damage = i64max(damage * md->damagetaken10 / 100, 1);
